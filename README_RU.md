...
Основные поля:
- `allowed_chat_id` — один ID Telegram-чата, которому разрешено работать с ботом
- `allowed_chat_ids` — необязательный список ID Telegram-чатов для многопользовательского доступа
