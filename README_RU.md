...
Локально тесты можно запускать так:

```bash
python3 -m unittest discover -s tests -v
```

Также в репозитории есть GitHub Actions workflow:

```text
.github/workflows/tests.yml
```

Он автоматически запускает тесты на каждый push и pull request.
...
Рекомендуемый подход для продового использования:

- использовать `config/*.local.json` для локальных runtime-настроек
- по возможности хранить секреты в переменных окружения
- запускать мониторы через systemd timers или другой планировщик
- обеспечить сервисному пользователю доступ на запись в `state/` и `logs/`
